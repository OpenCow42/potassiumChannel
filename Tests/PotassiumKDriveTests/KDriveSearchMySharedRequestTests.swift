...
@Suite("kDrive my-shared search requests")
struct KDriveSearchMySharedRequestTests {
    @Test("kDrive my-shared search request matches the OpenAPI path and query")
    func kDriveMySharedSearchRequestMatchesOpenAPIShape() async throws {
        ...
        let request = KDriveRequests.searchMyShared(
            driveId: 100,
            with: "categories",
            options: SearchKDriveMySharedOptions(
                cursor: "next",
                limit: 25,
        ...
        #expect(urlRequest.url?.path == "/3/drive/100/files/search/my_shared")
    ...
    @Test("kDrive my-shared search response decodes cursor metadata and file items")
    func kDriveMySharedSearchResponseDecodes() throws {
        let json = """
        {
          "result": "success",
          "data": [
