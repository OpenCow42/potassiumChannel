...
@Suite("kDrive file search requests")
struct KDriveFileSearchRequestTests {
    @Test("kDrive file search request matches the OpenAPI path and query")
    func kDriveFileSearchRequestMatchesOpenAPIShape() async throws {
        ...
        let request = KDriveRequests.searchFiles(
            driveId: 100,
            with: "categories",
            options: SearchKDriveFilesOptions(
                cursor: "next",
                limit: 25,
        ...
        #expect(urlRequest.url?.path == "/3/drive/100/files/search")
    ...
    @Test("kDrive file search response decodes cursor metadata and file items")
    func kDriveFileSearchResponseDecodes() throws {
        let json = """
        {
          "result": "success",
          "data": [
