...
@Suite("kDrive shared-with-me search requests")
struct KDriveSearchSharedWithMeRequestTests {
    @Test("kDrive shared-with-me search request matches the OpenAPI path and query")
    func kDriveSharedWithMeSearchRequestMatchesOpenAPIShape() async throws {
        ...
        let request = KDriveRequests.searchSharedWithMe(
            driveId: 100,
            with: "categories",
            options: SearchKDriveSharedWithMeOptions(
                cursor: "next",
                limit: 25,
        ...
        #expect(urlRequest.url?.path == "/3/drive/100/files/search/shared_with_me")
    ...
    @Test("kDrive shared-with-me search response decodes cursor metadata and file items")
    func kDriveSharedWithMeSearchResponseDecodes() throws {
        let json = """
        {
          "result": "success",
          "data": [
