...
@Suite("kDrive trash search requests")
struct KDriveSearchTrashRequestTests {
    @Test("kDrive trash search request matches the OpenAPI path and query")
    func kDriveTrashSearchRequestMatchesOpenAPIShape() async throws {
        ...
        let request = KDriveRequests.searchTrash(
            driveId: 100,
            with: "categories",
            options: SearchKDriveTrashOptions(
                cursor: "next",
                limit: 25,
                ...
                authorId: 10,
                category: "(1&2)|3",
                deletedAfter: 1710000000,
                deletedAt: "custom",
                deletedBefore: 1710003600,
                deletedBy: 11,
                query: "roadmap",
                queryScope: "all",
                types: ["file", "dir"]
        ...
        #expect(urlRequest.url?.path == "/3/drive/100/files/search/trash")
        ...
        #expect(queryItems.contains(URLQueryItem(name: "deleted_after", value: "1710000000")))
        #expect(queryItems.contains(URLQueryItem(name: "deleted_at", value: "custom")))
        #expect(queryItems.contains(URLQueryItem(name: "deleted_before", value: "1710003600")))
        #expect(queryItems.contains(URLQueryItem(name: "deleted_by", value: "11")))
    ...
    @Test("kDrive trash search response decodes cursor metadata and file items")
    func kDriveTrashSearchResponseDecodes() throws {
        let json = """
        {
          "result": "success",
          "data": [
        ...
        #expect(response.data.first?.isFavorite == false)
    }
}
