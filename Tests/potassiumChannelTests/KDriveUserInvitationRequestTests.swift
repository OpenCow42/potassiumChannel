...
struct KDriveUserInvitationRequestTests {
    ...
    @Test("kDrive user invitation detail request matches the OpenAPI path")
    func kDriveUserInvitationDetailRequestMatchesOpenAPIShape() async throws {
        let client = InfomaniakAPIClient(
            configuration: APIClientConfiguration(
                baseURL: URL(string: "https://api.infomaniak.com")!,
                bearerToken: "test-token"
        ...
        let request = KDriveRequests.getUserInvitation(driveId: 100, invitationId: 42)

        let urlRequest = try await client.makeURLRequest(for: request)

        #expect(urlRequest.httpMethod == "GET")
        #expect(urlRequest.value(forHTTPHeaderField: "Authorization") == "Bearer test-token")
        #expect(urlRequest.url?.path == "/2/drive/100/users/invitation/42")
    ...
    @Test("kDrive user invitation detail response decodes using the list DTO")
    func kDriveUserInvitationDetailResponseDecodes() throws {
        let json = """
        {
          "result": "success",
          "data": {
        ...
        let response = try decoder.decode(InfomaniakResponse<KDriveUserInvitation>.self, from: json)

        #expect(response.result == "success")
        #expect(response.data.id == 1)
        #expect(response.data.email == "invitee@example.com")
        #expect(response.data.status == "waiting")
    }
}
