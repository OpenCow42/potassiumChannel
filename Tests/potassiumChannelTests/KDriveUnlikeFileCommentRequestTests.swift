...
@Suite("kDrive unlike file comment request")
struct KDriveUnlikeFileCommentRequestTests {
    @Test("kDrive unlike file comment request matches the OpenAPI shape")
    func requestMatchesOpenAPIShape() async throws {
        ...
        let request = KDriveRequests.unlikeFileComment(
            driveId: 100,
            fileId: 42,
            commentId: "comment-7"
        )
        ...
        #expect(urlRequest.url?.path == "/2/drive/100/files/42/comments/comment-7/unlike")
        #expect(urlRequest.url?.query?.isEmpty ?? true)
        #expect(urlRequest.httpBody == nil)
    }

    @Test("unlike file comment required path parameters are encoded into the URL")
    func requiredPathParametersAreEncoded() async throws {
        ...
        let request = KDriveRequests.unlikeFileComment(driveId: 123, fileId: 456, commentId: "abc-789")

        let urlRequest = try await client.makeURLRequest(for: request)
        let path = try #require(urlRequest.url?.path)

        #expect(path == "/2/drive/123/files/456/comments/abc-789/unlike")
        #expect(!path.contains("{drive_id}"))
        #expect(!path.contains("{file_id}"))
        #expect(!path.contains("{comment_id}"))
    }

    @Test("unlike file comment response decodes boolean data")
