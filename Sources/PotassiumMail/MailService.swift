...
public struct MailService: Sendable {
    ...
    public func deleteDraft(mailboxUUID: String, draftUUID: String) async throws -> InfomaniakResponse<MailJSONValue> {
    ...
    public func deleteSchedule(scheduleAction: String) async throws -> InfomaniakResponse<MailJSONValue> {
        try await client.send(MailRequests.deleteSchedule(scheduleAction: scheduleAction))
    }

    /// Cancels a delayed send action.
    public func cancelSend(cancelSendResource: String) async throws -> InfomaniakResponse<MailJSONValue> {
